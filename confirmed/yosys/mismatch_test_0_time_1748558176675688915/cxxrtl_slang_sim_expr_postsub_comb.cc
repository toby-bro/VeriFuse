...
// \top: 1
// \src: ../expr_postsub_comb.sv:1.8
struct p_expr__postsub__comb : public module {
	// \src: ../expr_postsub_comb.sv:2.23
	/*input*/ value<8> p_in__val__m2;
	// \src: ../expr_postsub_comb.sv:4.24
	/*output*/ value<8> p_out__diff__m2;
	// \src: ../expr_postsub_comb.sv:3.23
	/*input*/ value<8> p_sub__val__m2;
	// \src: ../expr_postsub_comb.sv:5.24
...
bool p_expr__postsub__comb::eval(performer *performer) {
	...
	p_out__diff__m2 = sub_uu<8>(p_var__out__m2, p_sub__val__m2);
	return converged;
}
...
void p_expr__postsub__comb::debug_info(debug_items *items, debug_scopes *scopes, std::string path, metadata_map &&cell_attrs) {
	...
	if (items) {
		items->add(path, "in_val_m2", "src\000s../expr_postsub_comb.sv:2.23\000", p_in__val__m2, 0, debug_item::INPUT|debug_item::UNDRIVEN);
		items->add(path, "out_diff_m2", "src\000s../expr_postsub_comb.sv:4.24\000", p_out__diff__m2, 0, debug_item::OUTPUT|debug_item::DRIVEN_COMB);
		items->add(path, "sub_val_m2", "src\000s../expr_postsub_comb.sv:3.23\000", p_sub__val__m2, 0, debug_item::INPUT|debug_item::UNDRIVEN);
		items->add(path, "var_m2", "src\000s../expr_postsub_comb.sv:7.17\000", debug_alias(), p_var__out__m2);
		items->add(path, "var_out_m2", "src\000s../expr_postsub_comb.sv:5.24\000", p_var__out__m2, 0, debug_item::OUTPUT|debug_item::DRIVEN_COMB);
	}
}
