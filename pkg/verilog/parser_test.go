...
func TestSystemVerilogPackageImportPatterns(t *testing.T) {
	testContent := `
module test_imports(
    input logic [1:0] sel,
    input logic [3:0] data,
    ...
    import test_pkg::TEST_PARAM, other_pkg::OTHER_PARAM;
	...
	packageImports := MatchPackageImportsFromString(testContent)
	if len(packageImports) == 0 {
		t.Error("Expected to find package import statements")
	}
	...
	for _, match := range packageImports {
		if len(match) > 2 && match[1] == "uvm_pkg" && match[2] == "*" {
			importFound = true
			break
		}
	}
	if !importFound {
		t.Error("Expected to find 'import uvm_pkg::*' statement")
	}
}
